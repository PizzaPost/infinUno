...
if __name__ == "__main__":
    ...
    while running:
                ...
                if event.type == pygame.KEYDOWN:
                    if active:
                        if event.key == pygame.K_RETURN:
                            ...
                            if txt_surface2.get_width() > input_box.width - 10:
                                text = text[:-1]
        ...
        r, g, b = (
            ...
            255,
            255,
        )  # before we play the animation, initialize r, g, b to white in case it fails somehow. This is redundant, but it might help with a future -noanimation start condition.
        ...
        txt_surface1 = font1.render(
            "Enter the lobby password to create or join a game.", True, color
        )
        txt_surface2 = font1.render(text, True, color)
        window.window.blit(
            txt_surface0, (width // 2 - txt_surface0.get_width() // 2, height // 3)
        )
        window.window.blit(
            txt_surface1, (width // 2 - txt_surface1.get_width() // 2, input_box.y - 32)
        )
